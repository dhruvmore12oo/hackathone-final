...
export const Route = createFileRoute("/sign-up")({
  head: () => ({ meta: [{ title: "Sign up - FlowERP" }] }),
    ...
    <main className="min-h-screen bg-background text-foreground grid lg:grid-cols-[0.95fr_1.05fr]">
      <section className="hidden lg:flex flex-col justify-center items-center border-r border-border/70 bg-card/40 p-10 relative overflow-hidden">
        <div className="w-full max-w-md flex flex-col items-start -mt-20">
          ...
          <div className="w-full">
            <div className="text-xs font-medium uppercase tracking-wider text-primary">Secure Operations</div>
            <h1 className="mt-3 text-4xl font-semibold tracking-tight">Create your FlowERP workspace user.</h1>
            <p className="mt-4 text-sm leading-6 text-muted-foreground">
              New users will be synced into the ERP database and assigned permissions through the backend role layer.
            </p>
          </div>
      ...
      <section className="flex min-h-screen items-center justify-center px-6 py-10">
        <div className="w-full max-w-md flex flex-col items-center">
          ...
          <div className="w-full">
            <SignUp routing="path" path="/sign-up" signInUrl="/sign-in" fallbackRedirectUrl="/choose-role" />
          </div>
        </div>
      </section>
    </main>
